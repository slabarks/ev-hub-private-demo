...
import { buildMaturityModel, classifySiteRepeatability, dailyHistoryForSite, forecastSiteForward12M, forecastSiteMaturity } from "./engines/maturityEngine.js";
...
function portfolioCapexInfo(site, modelDayOneCapex = 0) {
  ...
  // Board convention: positive means adverse overspend; negative means favourable underspend.
  return { actual, model, note, variance: actual > 0 && Number.isFinite(model) ? actual - model : null };
}
...
const PORTFOLIO_STORED_ACTUALS_DATE = "2026-07-13";
function portfolioActualsDateDisplay(value) {
  const parsed = portfolioParseDate(value);
  if (!parsed) return "Date unavailable";
  return parsed.toLocaleDateString("en-IE", { day: "2-digit", month: "short", year: "numeric", timeZone: "UTC" });
}
function portfolioLiveCalibrationCard(sites = portfolioMappedSites()) {
  ...
  const latestRaw = portfolioLiveActualsSnapshot?.latestDate || PORTFOLIO_STORED_ACTUALS_DATE;
  const latest = portfolioActualsDateDisplay(latestRaw);
...
function portfolioMaturityDescription(value) {
  if (value && typeof value === "object" && value.key) {
    return {
      key: value.key,
      label: value.label || value.shortLabel || "Review",
      compact: value.shortLabel || value.label || "Review",
      title: value.label || "Maturity status",
      body: value.description || "Maturity classification is based on operating history, stability, volatility, data coverage and technical headroom.",
      cls: value.cls || "neutral",
      score: Number.isFinite(Number(value.score)) ? Number(value.score) : null,
      confidence: value.confidence || "",
      evidence: Array.isArray(value.evidence) ? value.evidence : [],
      disruptionSuspected: Boolean(value.disruptionSuspected),
      capacityConstrained: Boolean(value.capacityConstrained)
    };
  }
  const tier = String(value || "review");
  const map = {
    mature: {
      key: "repeatable",
      label: "Repeatable / mature",
      compact: "Repeatable",
      title: "Repeatable / mature site",
      body: "At least one seasonal year is available and recent seasonally adjusted performance is sufficiently stable for repeatable forecasting.",
      cls: "good"
    },
    near: {
      key: "stabilising",
      label: "Stabilising",
      compact: "Stabilising",
      title: "Stabilising site",
      body: "The site is approaching a repeatable run-rate, but history or recent stability is not yet sufficient for mature classification.",
      cls: "warn"
    },
    early: {
      key: "early_evidence",
      ...
      body: "Limited operating history means the run-rate is directional and not yet repeatable.",
      cls: "neutral"
    },
    review: {
      key: "review",
      ...
      compact: "Review",
      title: "Review site",
      body: "This site has incomplete or lower-confidence setup data and should not be used for automatic model conclusions.",
      cls: "neutral"
    }
  };
  return map[tier] || map.review;
}
function portfolioMaturityLabel(value) {
  return portfolioMaturityDescription(value).label;
}
function portfolioMaturityBadge(value) {
  const info = portfolioMaturityDescription(value);
  return `<button type="button" class="portfolio-maturity-trigger" aria-label="Explain ${h(info.label)} maturity" data-portfolio-maturity-trigger="1" data-maturity="${h(info.key || "review")}" data-title="${h(info.title)}" data-label="${h(info.label)}" data-description="${h(info.body)}" data-score="${Number.isFinite(info.score) ? h(String(info.score)) : ""}" data-confidence="${h(info.confidence || "")}" data-evidence="${h((info.evidence || []).join(" | "))}" data-disruption="${info.disruptionSuspected ? "1" : "0"}" data-capacity-constrained="${info.capacityConstrained ? "1" : "0"}"><span class="badge ${h(info.cls)}">${h(info.label)}</span></button>`;
}
function portfolioMaturityBadgeCompact(value) {
  const info = portfolioMaturityDescription(value);
  return `<button type="button" class="portfolio-maturity-trigger" aria-label="Explain ${h(info.label)} maturity" data-portfolio-maturity-trigger="1" data-maturity="${h(info.key || "review")}" data-title="${h(info.title)}" data-label="${h(info.label)}" data-description="${h(info.body)}" data-score="${Number.isFinite(info.score) ? h(String(info.score)) : ""}" data-confidence="${h(info.confidence || "")}" data-evidence="${h((info.evidence || []).join(" | "))}" data-disruption="${info.disruptionSuspected ? "1" : "0"}" data-capacity-constrained="${info.capacityConstrained ? "1" : "0"}"><span class="badge ${h(info.cls)}" title="${h(info.label)}">${h(info.compact || info.label)}</span></button>`;
}
...
function portfolioBenchmarksByCategory(sites = portfolioSites()) {
  ...
  const maturityModel = portfolioFinancialMaturityModel(benchmarkPool);
  const repeatability = new Map(benchmarkPool.map(site => [site.id || site.name, classifySiteRepeatability({ site, model: maturityModel })]));
  const isBenchmarkGrade = site => ["repeatable", "stabilising"].includes(repeatability.get(site.id || site.name)?.key);
  const benchmarks = {};
  Object.keys(PORTFOLIO_CATEGORY_FACTORS).forEach(categoryKey => {
    ...
    let sample = sameCategory.filter(isBenchmarkGrade);
    let sampleBasis = "repeatable + stabilising peers";
    if (sample.length < 2) {
      sample = sameCategory;
      sampleBasis = sample.length ? "all available peers" : "portfolio fallback";
    }
    if (sample.length < 2) {
      sample = benchmarkPool.filter(isBenchmarkGrade);
      sampleBasis = "all repeatable + stabilising benchmark sites fallback";
    }
...
function portfolioDoNothingPath(site, metrics, derived, inputs) {
  ...
  for (let t = 0; t < 20; t += 1) {
    ...
    const rawPlugRatio = installedOutputs > 0 ? installedOutputs / Math.max(1, peakConcurrentSessions) : 0;
    const rawMicRatio = gridPowerKw > 0 ? gridPowerKw / Math.max(1, peakDemandKw) : 0;
    const rawChargerRatio = installedPowerKw > 0 ? installedPowerKw / Math.max(1, peakDemandKw) : 0;
    const technicalCapacityMultiple = Math.max(0, Math.min(rawPlugRatio, rawMicRatio, rawChargerRatio));
    const plugRatio = Math.min(1, rawPlugRatio);
    const micRatio = Math.min(1, rawMicRatio);
    const chargerRatio = Math.min(1, rawChargerRatio);
    const capacityRatio = Math.min(1, technicalCapacityMultiple);
    const technicalCapacityAnnualKwh = annualKwhDemand * technicalCapacityMultiple;
...
function portfolioFilterValues(key) {
  ...
  if (key !== "maturity") return values;
  const legacyMap = { mature: "repeatable", near: "stabilising", early: "early_evidence", review: "declining_disrupted" };
  return [...new Set(values.map(value => legacyMap[value] || value))];
}
...
function portfolioApplyFilters(results) {
  return results.filter(r => {
    if (!portfolioFilterMatches("maturity", r.predictionMaturity?.key || r.site.maturity?.tier || "review")) return false;
...
function portfolioSortResults(results) {
  ...
  const getters = {
    site: r => r.site.name,
    maturity: r => portfolioMaturityLabel(r.predictionMaturity || r.site.maturity?.tier),
...
function showPortfolioMaturityPopover(button) {
  ...
  pop.innerHTML = `
    ...
    <div class="portfolio-status-popover-body">
      ...
      ${button.dataset.score ? `<div class="portfolio-status-popover-kpis"><span><small>Repeatability score</small><b>${h(button.dataset.score)}/100</b></span><span><small>Confidence</small><b>${h(button.dataset.confidence || "—")}</b></span></div>` : ""}
      ${button.dataset.evidence ? `<p><strong>Evidence:</strong> ${h(button.dataset.evidence).replace(/ \| /g, " · ")}</p>` : ""}
      ${button.dataset.disruption === "1" ? `<p class="notice small"><strong>Review:</strong> A possible operational or source-data disruption was detected. The forecast confidence is widened until the period is confirmed.</p>` : ""}
      ${button.dataset.capacityConstrained === "1" ? `<p class="notice small"><strong>Capacity:</strong> The installed system may be limiting delivered energy. Commercial potential and technical delivery capacity are treated separately.</p>` : ""}
      <p class="muted small">The classification uses operating history, complete months, data coverage, seasonally adjusted trend stability, robust volatility and technical headroom. It does not treat one strong month as proof of maturity.</p>
    </div>`;
  document.body.appendChild(pop);
  button.setAttribute("aria-expanded", "true");
  const closeBtn = pop.querySelector(".portfolio-status-popover-close");
...
function renderPortfolioCalibration() {
  ...
  const predictionMaturityModel = portfolioFinancialMaturityModel(mappedPortfolioSiteList);
  const results = mappedPortfolioSiteList.map(site => {
    const result = portfolioSiteResults(site, benchmarks);
    result.predictionMaturity = portfolioPredictionMaturity(site, result, predictionMaturityModel);
    return result;
  });
  const selectedResult = results.find(result => result.site?.id === selected?.id) || (() => {
    const result = portfolioSiteResults(selected, benchmarks);
    result.predictionMaturity = portfolioPredictionMaturity(selected, result, predictionMaturityModel);
    return result;
  })();
  ...
  const mature = results.filter(r => r.predictionMaturity?.key === "repeatable" && r.site.benchmarkEligible !== false);
  const near = results.filter(r => r.predictionMaturity?.key === "stabilising" && r.site.benchmarkEligible !== false);
  ...
  const row = r => [
    ...
    portfolioMaturityBadgeCompact(r.predictionMaturity || r.site.maturity?.tier),
  ...
  const maturityOptions = [
    {value:"early_evidence",label:"Early evidence"},
    {value:"ramping",label:"Ramping"},
    {value:"stabilising",label:"Stabilising"},
    {value:"repeatable",label:"Repeatable / mature"},
    {value:"late_ramping",label:"Late-ramping"},
    {value:"declining_disrupted",label:"Declining / disrupted"},
    {value:"capacity_constrained",label:"Capacity-constrained"}
  ];
...
function portfolioFinancialMaturityModel(sites = portfolioSites({ includeAdditional: false })) {
  const historySignature = sites.map(site => {
    ...
    const daily = Array.isArray(site?.actual?.dailyHistory) ? site.actual.dailyHistory : [];
    ...
    const sourceDays = daily.filter(row => row?.sourcePresent !== false).length;
    return `${site.id || site.name}:${Number(site?.maturity?.dataDays || 0)}:${history.length}:${Math.round(totalKwh)}:${latest?.month || latest?.monthStart || ""}:${Math.round(Number(latest?.kwh || 0))}:${daily.length}:${sourceDays}:${latestDaily?.date || ""}`;
  }).join("|");
  ...
  const predictionSites = sites.map(site => ({ ...site, categoryKey: portfolioCategoryKey(site) }));
  const model = buildMaturityModel(predictionSites, {
...
function portfolioPredictionMaturity(site, result, model = portfolioFinancialMaturityModel()) {
  const capacity = result?.doNothing?.rows?.[0] || null;
  return classifySiteRepeatability({
    site: { ...site, categoryKey: portfolioCategoryKey(site) },
    model,
    dataDays: Number(site?.maturity?.dataDays || site?.actual?.dataDays || site?.actual?.operationalDays || 0),
    technicalCapacityRatio: Number.isFinite(Number(capacity?.capacityRatio)) ? Number(capacity.capacityRatio) : null,
    technicalCapacityAnnualKwh: Number(capacity?.technicalCapacityAnnualKwh || 0) || null,
    capacityConstrained: Number(capacity?.capacityRatio || 1) < 0.98
  });
}
...
function portfolioFinancialMaturityForecast(site, result, annualActual, revenueInfo, operationalDays, model) {
  const predictionSite = { ...site, categoryKey: portfolioCategoryKey(site) };
  ...
  const technicalCapacityRow = result?.doNothing?.rows?.[0] || null;
  const commonParams = {
    site: predictionSite,
    model,
    ...
    technicalCapacityAnnualKwh: Number(technicalCapacityRow?.technicalCapacityAnnualKwh || 0) || null,
    technicalCapacityRatio: Number.isFinite(Number(technicalCapacityRow?.capacityRatio)) ? Number(technicalCapacityRow.capacityRatio) : null,
    capacityConstrained: Number(technicalCapacityRow?.capacityRatio || 1) < 0.98
  };
...
function portfolioCapexDeltaBand(delta) {
  ...
  const key = absolute <= 30000 ? "green" : absolute <= 50000 ? "amber" : "red";
  if (value > 0) return { key, cls: "capex-delta-red", label: `Overspend${absolute <= 30000 ? " · within €30k" : absolute <= 50000 ? " · €30k–€50k" : " · over €50k"}` };
  if (value < 0) return { key, cls: "capex-delta-green", label: `Underspend${absolute <= 30000 ? " · within €30k" : absolute <= 50000 ? " · €30k–€50k" : " · over €50k"}` };
  return { key: "green", cls: "neutral", label: "On budget" };
}
...
function portfolioHistoricalPerformanceStatus(actualKwh, modelKwh, periodDays) {
  const actual = Number(actualKwh);
  const model = Number(modelKwh);
...
function portfolioHistoricalModelBacktest(site, result, annualActual, operationalDays) {
  ...
  let periodEnd = null;
  let actualBasis = "Historical actual period";
  if (daily.length) {
    periodStart = portfolioParseDate(daily[0]?.date);
    periodEnd = portfolioParseDate(daily[daily.length - 1]?.date);
    actualKwh = daily.reduce((sum, row) => sum + Math.max(0, Number(row.kwh || 0)), 0);
    periodDays = periodStart && periodEnd ? Math.max(1, portfolioDateDiffDays(periodStart, periodEnd) + 1) : daily.length;
    actualBasis = "Uploaded daily site history";
  } else {
    ...
    if (!(actualKwh > 0) && annualActual?.isTrailing365Actual) {
      ...
      periodDays = 365;
      periodStart = periodEnd ? portfolioDateAddDays(periodEnd, -364) : periodStart;
      actualBasis = "Stored trailing-365 actual";
    } else if (!(actualKwh > 0) && Number(annualActual?.annualKwh || 0) > 0 && periodDays > 0) {
      actualKwh = Number(annualActual.annualKwh || 0) * periodDays / 365;
      actualBasis = "Stored cumulative actual reconstructed from annualised basis";
    } else {
      actualBasis = actualKwh > 0 ? "Stored cumulative actual" : actualBasis;
    }
  }
  ...
  if (site && inputs && periodDays > 0) {
    for (let index = 0; index < periodDays; index += 1) {
      const yearIndex = Math.max(1, Math.min(20, Math.floor((startAgeDays + index) / 365) + 1));
      yearDayCounts.set(yearIndex, (yearDayCounts.get(yearIndex) || 0) + 1);
    }
    yearDayCounts.forEach((days, yearIndex) => {
      const estimate = portfolioCalibratedAnnualEstimate(site, inputs, yearIndex);
      modelKwh += Math.max(0, Number(estimate?.modelKwh || 0)) * days / 365;
    });
  }
  const modelBasis = [...yearDayCounts.entries()].map(([yearIndex, days]) => `Y${yearIndex} ${days}d`).join(" / ") || "age-matched model unavailable";
  const periodLabel = periodStart && periodEnd ? `${portfolioDateLabel(periodStart)} → ${portfolioDateLabel(periodEnd)}` : `${number(periodDays,0)} commercial days`;
  const status = portfolioHistoricalPerformanceStatus(actualKwh, modelKwh, periodDays);
  return { actualKwh, modelKwh, variance: status.variance, status, periodDays, periodStart, periodEnd, periodLabel, actualBasis, modelBasis: `Current calibrated age-matched model · ${modelBasis}` };
}
...
const PORTFOLIO_MODEL_SNAPSHOT_PREFIX = "evHub.portfolio.modelSnapshot.v1";
function portfolioModelSnapshotKey(site) {
  return `${PORTFOLIO_MODEL_SNAPSHOT_PREFIX}.${portfolioCommercialTermsKey(site)}`;
}
function ensurePortfolioInvestmentSnapshot(site, result, maturityForecast) {
  const key = portfolioModelSnapshotKey(site);
  let existing = null;
  try { existing = JSON.parse(localStorage.getItem(key) || "null"); } catch (_) { existing = null; }
  if (existing?.capturedAt && existing?.modelBuildId) return existing;
  const snapshot = {
    snapshotType: "first-available-baseline",
    note: "Created automatically from the first model evaluation available after snapshot governance was introduced. It is not asserted to be the original historic investment approval case unless captured at approval time.",
    capturedAt: new Date().toISOString(),
    modelRelease: APP_RELEASE_VERSION,
    modelBuildId: APP_BUILD_ID,
    siteId: site?.id || "",
    siteName: site?.name || "Site",
    modelledAnnualKwh: Number(result?.modelledAnnualKwh || 0),
    modelComparisonBasis: result?.modelComparisonBasis || "",
    next12mKwh: Number(maturityForecast?.next12mKwh || 0),
    matureAnnualKwh: Number(maturityForecast?.matureAnnualKwh || 0),
    commercialPotentialAnnualKwh: Number(maturityForecast?.commercialPotentialAnnualKwh || 0),
    technicalCapacityAnnualKwh: Number(maturityForecast?.technicalCapacityAnnualKwh || 0),
    predictionEngineVersion: maturityModelVersion(maturityForecast),
    assumptions: {
      trafficGrowth: Number(state.inputs.annualTrafficGrowthRate ?? DEFAULT_INPUTS.annualTrafficGrowthRate ?? 0),
      tariffEscalation: Number(state.inputs.annualTariffEscalation ?? DEFAULT_INPUTS.annualTariffEscalation ?? 0),
      electricityEscalation: Number(state.inputs.annualElectricityCostEscalation ?? DEFAULT_INPUTS.annualElectricityCostEscalation ?? 0),
      averageSessionEnergy: Number(result?.inputs?.averageSessionEnergy ?? DEFAULT_INPUTS.averageSessionEnergy ?? 30.4),
      netSellingPriceExVat: Number(result?.inputs?.netSellingPriceExVat ?? state.inputs.netSellingPriceExVat ?? DEFAULT_INPUTS.netSellingPriceExVat ?? 0),
      selectedMicKva: Number(result?.config?.selectedMicKva || site?.realMicKva || 0)
    }
  };
  try { localStorage.setItem(key, JSON.stringify(snapshot)); } catch (_) {}
  return snapshot;
}
function maturityModelVersion(maturityForecast) {
  return maturityForecast?.forward12m?.modelType || maturityForecast?.label || "prediction-engine";
}
...
function portfolioFinancialRow(site, benchmarks, maturityModel) {
  ...
  const investmentCaseSnapshot = ensurePortfolioInvestmentSnapshot(site, result, maturityForecast);
  ...
  const forwardPerformanceStatus = portfolioFinancialPerformanceStatus(maturityForecast.next12mKwh, modelForward12m.kwh);
  const historicalBacktest = portfolioHistoricalModelBacktest(site, result, annualActual, operationalDays);
  const performanceStatus = historicalBacktest.status;
  const actualComparableKwh = Math.max(0, Number(annualActual.annualKwh || 0));
  const forecastVsActualVariance = actualComparableKwh > 0 ? (Number(maturityForecast.next12mKwh || 0) - actualComparableKwh) / actualComparableKwh : null;
  const fin = {
    result,
    site,
    ...
    historicalActualKwh: historicalBacktest.actualKwh,
    historicalModelKwh: historicalBacktest.modelKwh,
    historicalPeriodDays: historicalBacktest.periodDays,
    historicalPeriodLabel: historicalBacktest.periodLabel,
    historicalActualBasis: historicalBacktest.actualBasis,
    historicalModelBasis: historicalBacktest.modelBasis,
    modelForward12mKwh: modelForward12m.kwh,
    modelForward12mBasis: modelForward12m.basis,
    forwardPerformanceVariance: forwardPerformanceStatus.variance,
    forwardPerformanceStatus,
    performanceVariance: performanceStatus.variance,
    performanceStatus,
    matureAnnualKwh: maturityForecast.matureAnnualKwh,
    matureAnnualKwhP25: maturityForecast.matureAnnualKwhP25,
    matureAnnualKwhP75: maturityForecast.matureAnnualKwhP75,
    commercialPotentialAnnualKwh: maturityForecast.commercialPotentialAnnualKwh,
    technicalCapacityAnnualKwh: maturityForecast.technicalCapacityAnnualKwh,
    capacityConstrained: maturityForecast.capacityConstrained,
    predictionMaturity: maturityForecast.classification,
    maturityForecast,
...
function portfolioFinancialFilterPanel(rows, filteredRows) {
  ...
  return `<section class="panel portfolio-financial-filter-panel portfolio-financial-filter-panel-open"><div class="portfolio-financial-filter-toolbar"><span><strong>Filters</strong><small>${number(filteredRows.length,0)} of ${number(rows.length,0)} active sites selected${activeCount ? ` · ${number(activeCount,0)} filter${activeCount === 1 ? "" : "s"} active` : " · all sites"}</small></span><div class="portfolio-financial-toolbar-actions"><button type="button" class="secondary mini" data-portfolio-financial-reset-filters="1" ${activeCount ? "" : "disabled"}>Reset filters</button></div></div><p class="muted small">Filters change which sites are displayed. Portfolio-wide landlord and funding controls are available beside the main financial table heading.</p><div class="portfolio-financial-filter-grid">${defs.map(def => portfolioFinancialFilterSelect(def, rows)).join("")}</div></section>`;
}
...
function portfolioFundingBulkSummary(rows) {
  const funded = rows.filter(row => Number(row.fundingAvailable || 0) > 0);
  const confirmed = funded.filter(row => !row.funding?.reviewRequired);
  const review = funded.filter(row => row.funding?.reviewRequired);
  const exceedsCapex = funded.filter(row => row.hasActualCapex && Number(row.fundingAvailable || 0) > Number(row.actualCapex || 0));
  const applicableConfirmed = confirmed.filter(row => row.hasActualCapex && Number(row.fundingAvailable || 0) <= Number(row.actualCapex || 0));
  const applied = funded.filter(row => row.fundingApplied);
  return {
    funded, confirmed, review, exceedsCapex, applicableConfirmed, applied,
    availableTotal: funded.reduce((sum, row) => sum + Number(row.fundingAvailable || 0), 0),
    applicableTotal: applicableConfirmed.reduce((sum, row) => sum + Number(row.fundingAvailable || 0), 0),
...
function portfolioCommercialBulkModal(rows) {
  ...
  return `<div class="commercial-modal-backdrop" data-commercial-modal-backdrop="1"><section class="commercial-modal commercial-bulk-modal portfolio-terms-modal" role="dialog" aria-modal="true" aria-label="Portfolio terms manager">
    ...
    ${anomalyNames.length ? `<div class="notice warn"><strong>Funding exceeds recorded gross CAPEX</strong><br>${h(anomalyNames.join(", "))}. These sites are not bulk-applied; review the CAPEX scope and funding record individually.</div>` : ""}
    <div class="commercial-modal-actions funding-bulk-actions"><button type="button" class="secondary" data-funding-bulk-action="reset-database">Reset all to database values</button><button type="button" class="secondary" data-funding-bulk-action="remove-all">Remove funding from all</button><button type="button" class="primary" data-funding-bulk-action="apply-confirmed">Apply funding to all eligible sites</button></div></section>
    <div class="commercial-modal-actions portfolio-terms-close-row"><button type="button" class="secondary" data-commercial-terms-close="1">Close</button></div>
  </section></div>`;
}
...
function portfolioFinancialMaturitySummary(model, rows) {
  ...
  const empiricalMatureRows = usable.filter(row => {
    const classification = row.maturityForecast?.classification?.key;
    if (classification) return classification === "repeatable";
  ...
  const insufficientHistoryRows = usable.filter(row => row.maturityForecast?.classification?.key === "early_evidence" || historyMonths(row) < 2);
  const rampingRows = usable.filter(row => {
    const classification = row.maturityForecast?.classification?.key;
    if (classification) return ["ramping", "stabilising", "late_ramping"].includes(classification) && !insufficientHistoryRows.includes(row);
    return !empiricalMatureRows.includes(row) && !insufficientHistoryRows.includes(row);
  });
...
function portfolioFinancialDashboardWindows(rows, filteredRows, summary, projection, horizon) {
  ...
  const investment = portfolioFinancialDashboardWindow("Investment position", "Actual day-one build cost against the modelled complete day-one build for the selected sites.", [
    ...
    portfolioFinancialDashboardMetric("CAPEX Δ", `${summary.capexDelta > 0 ? "+" : ""}${currency(summary.capexDelta,0)}`, `actual minus model · ${capexDeltaNote}`, summary.capexDelta < 0 ? "capex-direction-negative" : summary.capexDelta > 0 ? "capex-direction-positive" : "capex-direction-neutral"),
    ...
    portfolioFinancialDashboardMetric("CAPEX Δ %", capexDeltaPct === null ? "—" : `${capexDeltaPct > 0 ? "+" : ""}${pct(capexDeltaPct,1)}`, "(actual minus model) / model", capexDeltaPct < 0 ? "capex-direction-negative" : capexDeltaPct > 0 ? "capex-direction-positive" : "capex-direction-neutral")
  ]);
...
function portfolioFinancialSummary(rows) {
  ...
  return {
    ...
    capexDelta: actualCapex - modelCapexForCapexRows,
    annualKwh,
    annualRevenue,
    annualOpex,
    electricityCost,
...
function portfolioFinancialExportPayload() {
  ...
  const dictionaryRows = [["Column", "Meaning"],
    ...
    ["Historical model performance", "Actual delivered kWh to date minus the current calibrated age-matched model, divided by that model. Above +15% is above model; below -15% is below model. Sites with less than 30 days remain review/early evidence."],
    ["Forward outlook vs benchmark", "Next-12-month actual-led forecast minus the current calibrated forward benchmark, divided by that benchmark. This remains visible in the forecast audit but is secondary to realised historical model accuracy."],
...
function portfolioFinancialTableRow(fin) {
  ...
  const capexTitle = Number.isFinite(delta) ? `${fin.capexDeltaBand?.label || "CAPEX variance"}. Actual gross day-one CAPEX minus complete modelled day-one CAPEX is ${delta > 0 ? "+" : ""}${currency(delta,0)} (${deltaPct === null ? "n/a" : `${deltaPct > 0 ? "+" : ""}${pct(deltaPct,1)}`}). Positive is an adverse overspend; negative is a favourable underspend. Future replacements, later plugs and progressive CAPEX are excluded.` : "CAPEX delta unavailable.";
  const fundingLine = fin.fundingAvailable > 0
    ...
    : "no matched funding";
  const deltaAmountLabel = Number.isFinite(delta) ? `${delta > 0 ? "+" : ""}${currency(delta,0)}` : "—";
  const deltaPctLabel = deltaPct === null ? "—" : `${deltaPct > 0 ? "+" : ""}${pct(deltaPct,1)}`;
  const capexSub = Number.isFinite(delta) ? `model ${currency(fin.modelCapex,0)} · <span class="${directionCls}">Δ ${deltaAmountLabel} · ${deltaPctLabel}</span><br><span class="funding-cell-note ${fin.fundingApplied ? "applied" : ""}">${h(fundingLine)}</span>` : `delta unavailable<br><span class="funding-cell-note">${h(fundingLine)}</span>`;
...
function wirePage(r) {
  ...
  document.querySelectorAll("[data-funding-bulk-action]").forEach(node => {
    node.addEventListener("click", e => {
      const action = e.currentTarget.dataset.fundingBulkAction;
      const rows = portfolioFinancialRows();
      const includeReview = Boolean(document.querySelector("[data-funding-bulk-include-review]")?.checked);
      if (action === "apply-confirmed") {
        const eligible = rows.filter(row => Number(row.fundingAvailable || 0) > 0 && row.hasActualCapex && Number(row.fundingAvailable || 0) <= Number(row.actualCapex || 0) && (!row.funding?.reviewRequired || includeReview));
        if (!eligible.length) return window.alert("No eligible funding records are available for bulk application.");
        if (!window.confirm(`Apply ${currency(eligible.reduce((sum,row)=>sum+Number(row.fundingAvailable||0),0),0)} of funding across ${eligible.length} site${eligible.length === 1 ? "" : "s"}?`)) return;
        eligible.forEach(row => portfolioFundingSave(row.site, { amount: row.fundingAvailable, applied: true, source: row.funding?.source, scheme: row.funding?.scheme, confidence: row.funding?.confidence, manualOverride: row.funding?.manualOverride }));
      } else if (action === "remove-all") {
        const funded = rows.filter(row => Number(row.fundingAvailable || 0) > 0);
        if (!window.confirm(`Remove applied funding from all ${funded.length} funded site${funded.length === 1 ? "" : "s"}? Funding values will remain available.`)) return;
        funded.forEach(row => portfolioFundingSave(row.site, { amount: row.fundingAvailable, applied: false, source: row.funding?.source, scheme: row.funding?.scheme, confidence: row.funding?.confidence, manualOverride: row.funding?.manualOverride }));
      } else if (action === "reset-database") {
        if (!window.confirm("Reset all funding amounts and choices to the database matches? All funding will be left unapplied.")) return;
        rows.forEach(row => portfolioFundingClear(row.site));
      }
      preserveScrollRender();
    });
  });
