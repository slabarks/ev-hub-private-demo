...
assert.match(app, /V17\.44 browser provenance-controlled AADT engine/);
assert.match(server, /V17\.44 AADT audited resolver/);
...
assert.match(server, /LIVE_UPLOAD_SCHEMA_VERSION = "v17\.44-live-history-v5"/);
assert.match(server, /APP_BUILD_ID = "EVHUB-V17\.44-20260710-R1"/);
assert.match(server, /LIVE_UPLOAD_PARSER_BUILD_ID = "EVHUB-LIVE-PARSER-17\.44\.1"/);
assert.match(server, /"monthlyObservationCount": monthly_observation_count/);
assert.match(app, /PORTFOLIO_LIVE_ACTUALS_SCHEMA_VERSION = "v17\.44-live-history-v5"/);
assert.match(app, /APP_BUILD_ID = "EVHUB-V17\.44-20260710-R1"/);
...
assert.match(app, /low-missing-history/);
...
assert.match(app, /Early \/ limited data/);
...
assert.match(exportEngine, /Early \/ limited data/);
...
assert.match(fs.readFileSync(path.join(root, "index.html"), "utf8"), /17\.44-upload-restore-20260710-r1/);
...
assert.match(app, /Restore the reliable workflow used in earlier releases/);
const snapshotContext = {};
vm.runInNewContext(`const APP_RELEASE_VERSION = "V17.44"; const APP_BUILD_ID = "EVHUB-V17.44-20260710-R1"; const LIVE_UPLOAD_PARSER_BUILD_ID = "EVHUB-LIVE-PARSER-17.44.1"; const PORTFOLIO_LIVE_ACTUALS_SCHEMA_VERSION = "v17.44-live-history-v5"; ${compatibilitySource}; ${snapshotValidationSource}; this.validate = portfolioSnapshotValidation;`, snapshotContext);
const buildMeta = { buildId: "EVHUB-V17.44-20260710-R1", uploadSchemaVersion: "v17.44-live-history-v5", parserBuildId: "EVHUB-LIVE-PARSER-17.44.1", monthlyHistorySupported: true, deploymentRootOk: true, packageLayoutVersion: "flat-root-v1", serverFileFingerprint: "testfingerprint" };
assert.equal(snapshotContext.validate({ ...buildMeta, parsedFiles: ["Daily_Charger_kWh.xlsx"], siteActuals: [{ actual: { annualKwh: 1000, monthlyHistory: [] } }] }).ok, true, "valid run-rate actuals must remain usable when monthly history is absent");
...
assert.equal(limitedHistory.note, "No completed monthly history");
...
try {
  ...
  assert.match(version.aadt_engine_version, /V17\.44/);
  assert.equal(version.buildId, "EVHUB-V17.44-20260710-R1");
  assert.equal(version.uploadSchemaVersion, "v17.44-live-history-v5");
  assert.equal(version.parserBuildId, "EVHUB-LIVE-PARSER-17.44.1");
  ...
  assert.equal(health.buildId, "EVHUB-V17.44-20260710-R1");
  ...
  const uploadResp = await fetch(`http://127.0.0.1:${port}/api/import-live-calibration`, { method: "POST", body: uploadForm });
  assert.equal(uploadResp.status, 200);
  const upload = await uploadResp.json();
  assert.equal(upload.ok, true);
  assert.equal(upload.buildId, "EVHUB-V17.44-20260710-R1");
  assert.equal(upload.schemaVersion, "v17.44-live-history-v5");
  assert.equal(upload.parserBuildId, "EVHUB-LIVE-PARSER-17.44.1");
  ...
  assert.doesNotMatch(indexText, /<title>[^<]*V17\.44/i);
...
console.log("\n[6/6] Result");
console.log("PASS — V17.44 AADT, forward performance benchmark, 1–20-year horizon, restored upload compatibility, exports and API smoke tests completed successfully.");
