...
APP_VERSION = "V21.4"
APP_BUILD_ID = "EVHUB-V21.4-20260719-R1"
LIVE_UPLOAD_SCHEMA_VERSION = "v21-live-history-v7"
LIVE_UPLOAD_PARSER_BUILD_ID = "EVHUB-LIVE-PARSER-21.5"
AADT_ENGINE_VERSION = "V21.4 AADT audited resolver + browser-local live-history upload"
...
def _deployment_integrity() -> dict:
    ...
    if index_path.exists():
        try:
            ...
            if "21-4-board-performance-20260719-r1" not in index_text:
                problems.append("index.html cache-buster is not the V21.4 deployment build")
        except Exception as exc:
...
def classify_urban_service_subcategory(site: dict, traffic: dict, live_chargers: list) -> dict:
    ...
    Automatically classify an urban-service-station address as either:
      - "town_hub_forecourt"  (target 0.45 sess/1kAADT) when evidence suggests the
        site serves an uncontested town catchment wider than the AADT corridor, or
      - "urban_service"       (target 0.19 sess/1kAADT) for bypass/corridor sites.

    Two signals, both derived from data already fetched during a normal site screen:

    Signal 1 — Competition (Overpass result already in live_chargers):
        Count DC-capable chargers (>= 50 kW or socket:type2_combo / socket:chademo)
        within 15 km that are NOT the site itself. Zero competitors is the
        strongest town-hub signal.

    Signal 2 — TII counter corridor descriptor (already in traffic dict):
        If the matched TII counter description contains bypass/motorway/junction
        keywords the site is on a through-route, which favours urban_service.

    Scoring:
        competition_score: +2 if 0 competitors, +1 if 1, 0 if 2+
        corridor_score:    -1 if bypass/motorway/Jn keyword in TII description
        total >= 2  → town_hub_forecourt  (high confidence)
        total == 1  → town_hub_forecourt  (medium confidence)
        total <= 0  → urban_service
    """
    import re as _re

    # --- Signal 1: DC competition within 15 km ---
    site_lat = float(site.get("lat") or 0)
    site_lon = float(site.get("lon") or 0)
    DC_KW_THRESHOLD = 50.0
    DC_RADIUS_KM = 15.0
    DC_KEYWORDS = ["type2_combo", "chademo", "ccs", "type2combo", "dc", "fast"]

    competitor_count = 0
    for ch in (live_chargers or []):
        dist = float(ch.get("distance_km") or 999)
        if dist < 0.05:          # skip self (< 50 m)
            continue
        if dist > DC_RADIUS_KM:
            continue
        # Check if it has DC capability from OSM tags
        connectors = ch.get("connectors") or []
        is_dc = False
        for c in connectors:
            c_str = str(c).lower()
            if any(k in c_str for k in DC_KEYWORDS):
                is_dc = True
                break
            kw = c.get("power_kw") or c.get("powerKw") or 0
            if float(kw or 0) >= DC_KW_THRESHOLD:
                is_dc = True
                break
        if is_dc:
            competitor_count += 1
    ...
    # --- Signal 2: TII counter description corridor keywords ---
    desc = str(traffic.get("description") or traffic.get("counter_name") or "").lower()
    counter_id = str(traffic.get("counter_id") or "").upper()
    BYPASS_KEYWORDS = ["bypass", "motorway", " jn", "jn.", "/m0", "/m1", "/m2", "/m3",
                       "/m4", "/m5", "/m6", "/m7", "/m8", "/m9", "/m10", "/m11", "/m17", "/m18",
                       " m50", "ring road", "outer ring", "dual carriageway"]
    is_bypass_corridor = any(kw in desc for kw in BYPASS_KEYWORDS)
    # M-roads in counter_id also indicate motorway
    if _re.search(r'M\d+', counter_id):
        is_bypass_corridor = True
    corridor_score = -1 if is_bypass_corridor else 0

    total = competition_score + corridor_score

    if total >= 2:
        confidence = "high"
        category = "town_hub_forecourt"
    elif total == 1:
        confidence = "medium"
        category = "town_hub_forecourt"
    else:
        confidence = "corridor match" if is_bypass_corridor else "competitive"
        category = "urban_service"

    basis_parts = []
    basis_parts.append(f"{competitor_count} DC competitor(s) within {DC_RADIUS_KM:.0f} km")
    basis_parts.append("bypass/motorway corridor detected" if is_bypass_corridor else "non-bypass road type")

    return {
        "suggestedCategoryKey": category,
        "suggestedCategoryLabel": "Town hub / community forecourt" if category == "town_hub_forecourt" else "Urban service station",
        "categoryAutoClassified": True,
        "categoryConfidence": confidence,
        "categoryBasis": "; ".join(basis_parts),
        "dcCompetitorCount": competitor_count,
        "isBypassCorridor": is_bypass_corridor,
    }
...
def search_coordinates(lat, lon, radius_km, address="Manual map point"):
    ...
    try:
        addr_lower = str(address or site.get("name", "")).lower()
        is_service_station = any(k in addr_lower for k in [
            "centra", "circle k", "corrib oil", "texaco", "topaz", "esso", "applegreen",
            "service station", "filling station", "forecourt", "petrol"
        ]) or (traffic.get("aadt", 0) < 40000 and "hotel" not in addr_lower and "retail" not in addr_lower)
...
def main():
    ...
    if selected_port != requested_port:
        print(f"Port {requested_port} was already in use. V21.4 selected {selected_port} to avoid opening a stale application instance.")
