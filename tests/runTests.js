...
assert.match(server, /V21 AADT audited resolver/);
...
assert.match(server, /APP_BUILD_ID = "EVHUB-V21-20260716-R1"/);
assert.match(server, /LIVE_UPLOAD_PARSER_BUILD_ID = "EVHUB-LIVE-PARSER-21\.1"/);
...
assert.match(app, /APP_BUILD_ID = "EVHUB-V21-20260716-R1"/);
...
assert.match(fs.readFileSync(path.join(root, "index.html"), "utf8"), /21-portfolio-forecast-audit-20260716-r1/);
...
vm.runInNewContext(`const APP_RELEASE_VERSION = "V21"; const APP_BUILD_ID = "EVHUB-V21-20260716-R1"; const LIVE_UPLOAD_PARSER_BUILD_ID = "EVHUB-LIVE-PARSER-21.1"; const PORTFOLIO_LIVE_ACTUALS_SCHEMA_VERSION = "v21-live-history-v7"; ${compatibilitySource}; ${snapshotValidationSource}; this.validate = portfolioSnapshotValidation;`, snapshotContext);
const buildMeta = { buildId: "EVHUB-V21-20260716-R1", uploadSchemaVersion: "v21-live-history-v7", parserBuildId: "EVHUB-LIVE-PARSER-21.1", monthlyHistorySupported: true, dailyHistorySupported: true, deploymentRootOk: true, packageLayoutVersion: "flat-root-v1", serverFileFingerprint: "testfingerprint" };
...
assert.equal(snapshotContext.validate({ ...buildMeta, buildId: "OLD", parsedFiles: ["Daily_Charger_kWh.xlsx"], siteActuals: [{ actual: validHistory }] }).ok, true, "backend build metadata must not block valid parsed data");
...
try {
  ...
  assert.equal(version.buildId, "EVHUB-V21-20260716-R1");
  assert.equal(version.uploadSchemaVersion, "v21-live-history-v7");
  assert.equal(version.parserBuildId, "EVHUB-LIVE-PARSER-21.1");
  ...
  assert.equal(health.buildId, "EVHUB-V21-20260716-R1");
  ...
  assert.equal(upload.buildId, "EVHUB-V21-20260716-R1");
  assert.equal(upload.schemaVersion, "v21-live-history-v7");
  assert.equal(upload.parserBuildId, "EVHUB-LIVE-PARSER-21.1");
...
console.log("\n[6/6] Result");
console.log("PASS — V21 upload integrity, reconciled performance status, 1–20-year horizon, canonical Portfolio Financials rendering, exports and API smoke tests completed successfully.");
