...
assert.match(server, /V21\.5 AADT audited resolver/);
...
assert.ok(v213CssMarker > css.lastIndexOf("width: 1650px"), "V21.5 fluid-width rules must override legacy fixed-width rules");
...
assert.match(server, /APP_BUILD_ID = "EVHUB-V21.5-20260719-R1"/);
...
assert.match(app, /APP_BUILD_ID = "EVHUB-V21.5-20260719-R1"/);
...
assert.match(fs.readFileSync(path.join(root, "index.html"), "utf8"), /21-5-investor-table-20260719-r1/);
...
vm.runInNewContext(`const APP_RELEASE_VERSION = "V21.5"; const APP_BUILD_ID = "EVHUB-V21.5-20260719-R1"; const LIVE_UPLOAD_PARSER_BUILD_ID = "EVHUB-LIVE-PARSER-21.5"; const PORTFOLIO_LIVE_ACTUALS_SCHEMA_VERSION = "v21-live-history-v7"; ${compatibilitySource}; ${snapshotValidationSource}; this.validate = portfolioSnapshotValidation;`, snapshotContext);
const buildMeta = { buildId: "EVHUB-V21.5-20260719-R1", uploadSchemaVersion: "v21-live-history-v7", parserBuildId: "EVHUB-LIVE-PARSER-21.5", monthlyHistorySupported: true, dailyHistorySupported: true, deploymentRootOk: true, packageLayoutVersion: "flat-root-v1", serverFileFingerprint: "testfingerprint" };
...
try {
  ...
  assert.equal(version.buildId, "EVHUB-V21.5-20260719-R1");
  ...
  assert.equal(prefixedVersion.buildId, "EVHUB-V21.5-20260719-R1");
  ...
  assert.equal(health.buildId, "EVHUB-V21.5-20260719-R1");
  ...
  assert.equal(upload.buildId, "EVHUB-V21.5-20260719-R1");
  ...
  assert.equal(uploadResp.headers.get("x-evhub-build"), "EVHUB-V21.5-20260719-R1");
...
console.log("\n[7/7] Result");
console.log("PASS — V21.5 browser-local upload integrity, backend fallback resilience, compact investor table cards, confirmed funding, 1–20-year horizon, exports and API smoke tests completed successfully.");
