...
const fetchMock = async (url, options = {}) => {
  ...
  if (href === "https://example.test/tools/evhub/api/import-live-calibration") {
    if (uploadMode === "relative-success") return Response.json({ ok: true, siteActuals: [{ actual: { annualKwh: 1 } }] });
    return new Response("not found", { status: 404, headers: { "content-type": "text/html" } });
  }
  if (href === "https://example.test/api/import-live-calibration") {
    return Response.json({ ok: true, siteActuals: [{ actual: { annualKwh: 2 } }] });
  }
  return new Response("not found", { status: 404, headers: { "content-type": "text/html" } });
};
...
console.log("PASS — missing version route no longer blocks upload; relative and root API discovery both work.");
